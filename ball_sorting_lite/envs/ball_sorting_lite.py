...
nok_reward = -1
empty_reward = -0.1
wrong_action_reward = -0.1
delay_multiplier = 0.1
ministate_activation = 0
percentage_reward = 10
...
class BallSortingLiteEnv(gym.Env):
    """
...
"""
    ...
    def __init__(self):
        ...
        self.start_type = random_init
        for i in range(n_rows):
                for j in range(n_cols):
                        # random initialization
                        if self.start_type == True:
        ...
        self.steps_remaining = self.episode_steps
        self.reward = 0
        self.done = False
        self.info = None
        self.action_space = gym.spaces.Discrete(2*3*3)
        self.observation_space = gym.spaces.Box(0,3,[ n_rows, n_cols], dtype=np.uint8)
    ...
    def evaluate_reward(self):
        ...
        try:                 
                ...
                self.tasa_aciertos = 0
        if self.percentage_reward >0:        
                self.reward += (self.tasa_aciertos-0.3333)*self.percentage_reward 
